  ...
  <div>
    ...
    <nav class="navbar d-flex" id="nav">
      <a class="navbar-brand" href="/">
        <img src="@/assets/cosmos_logo1.png" alt="cosmos" width="30" height="24">
      ...
      <div class="nav ml-auto align-items-center" v-if="this.$store.state.isLogin">
        ...
        <router-link :to="{ name:'StudyRoomCreateForm' }" class="text-decoration-none mx-1">스터디 방 생성</router-link>
        <router-link :to="{ name:'MyPage' }" class="text-decoration-none mx-1">마이페이지</router-link>
        <b-button class="m-1" @click="logOut">로그아웃</b-button>
      </div>
      <div class="nav ml-auto" v-else>
        <router-link :to="{ name:'SignUp' }" class="text-decoration-none mx-1">회원가입</router-link>
        <router-link :to="{ name:'LogIn' }" class="text-decoration-none mx-1">로그인</router-link>
      </div>
    </nav>
  </div>
</template>
